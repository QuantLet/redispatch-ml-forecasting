...
def mae_loss(actual: pd.Series, forecasts: pd.DataFrame):
    return np.abs(forecasts.sub(actual, axis=0))


def compare_models_mcs(predictions_df: pd.DataFrame, loss_func: Callable = mae_loss, alpha: float = 0.25, start_date: str | None = None, end_date: str | None = None) -> pd.DataFrame:
    results = []
    ...
    for tso, direction in predictions_df[["tso", "unique_id"]].drop_duplicates().itertuples(index=False):
        tso_data = predictions_df[np.logical_and(
            predictions_df["tso"] == tso,
            predictions_df["unique_id"] == direction
        )]
        if start_date is not None:
            tso_data = tso_data[tso_data["ds"] >= start_date]
        if end_date is not None:
            tso_data = tso_data[tso_data["ds"] <= end_date]
        if tso_data.empty:
            continue
        actual = tso_data["y"].reset_index(drop=True)
        forecast = tso_data[list(common_models)].reset_index(drop=True)
        mcs = ModelConfidenceSet(
            losses=loss_func(actual, forecast),
            alpha=alpha,
            n_boot=5_000,
            method="R",
        )
        mcs.compute()
        mcs_results = pd.DataFrame(mcs.results(as_dataframe=False)).assign(
            tso=tso,
            direction=direction,
        )
        results.append(mcs_results)
